...
//funciion para obtener los tipos Empresas
	public function gettiposEmpresa(){
		$sql=$this->db->select("*")->get("tiposempresa");
		return $sql->result();
	}
    //funciion para obtener los tipos Facturacion

   public function gettipoFacturacion(){
		$sql=$this->db->select("*")->get("tipfacturacion");
          return $sql->result();
	}

    //funciion para obtener los tipos Nº de empleados

 public function gettipoNempleados(){
 		$sql=$this->db->select("*")->get("noempleados");
 		 return $sql->result();
	}

    //funciion para obtener los tipos Estados

 public function gettipoEstado(){
		$sql=$this->db->select("*")->where("IDPais='42'")->get("estado");
 		 return $sql->result();
	}
